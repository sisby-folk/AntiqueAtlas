...
public class BiomeTextures extends JsonDataLoader implements IdentifiableResourceReloadListener {
    ...
    public void registerFallback(Identifier id, RegistryEntry<Biome> biome) {
        ...
        if (textureSet.isPresent()) {
            ...
            AntiqueAtlas.LOGGER.info("Auto-registered standard texture set for biome {}: {}", id, textureSet.get());
        } else {
            AntiqueAtlas.LOGGER.error("Failed to auto-register a standard texture set for the biome '{}'. This is most likely caused by errors in the TextureSet configurations, check your resource packs first before reporting it as an issue!", id.toString());
